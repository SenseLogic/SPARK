...
for %%f in (static\video\%1\*.mp4) do (
    ...
    %TOOL%\REMIX\remix %2 %TOOL%\IMAGE_MAGICK\convert "..\www\static\image\%1\%%~nf_video_thumbnail.avif" -resize 360x720 -quality 30 -filter Lanczos -strip "..\www\static\image\%1\%%~nf_video_thumbnail.avif.preload.avif"
)
