...
for %%f in (static\video\%1\*.mp4) do (
    ...
    ..\%TOOL%\REMIX\remix %2 ..\%TOOL%\IMAGE_MAGICK\convert "..\www\static\image\%1\%%~nf_video_thumbnail.gif[0]" -background white -alpha remove -alpha off -resize %5@ -interlace Plane -quality 50 -strip "..\www\static\image\%1\%%~nf_video_thumbnail.gif.preload.jpg"
    )
