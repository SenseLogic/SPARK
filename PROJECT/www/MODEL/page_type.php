    ...
    )
{
    ...
    while (  $page_type = $statement->fetchObject() )
    {
    ...
    )
{
    ...
    if ( !$statement->execute() )
    {
     ...
     $page_type = $statement->fetchObject();
    $page_type->Id = ( int )( $page_type->Id );
...
{
     $statement = GetDatabaseStatement( 'insert into `PAGE_TYPE` ( `Slug`, `Title` ) values ( ?, ? )' );
...
{
     $statement = GetDatabaseStatement( 'replace into `PAGE_TYPE` ( `Slug`, `Title` ) values ( ?, ? )' );
    $statement->bindParam( 1, $slug, PDO::PARAM_STR );
    $statement->bindParam( 2, $title, PDO::PARAM_STR );
